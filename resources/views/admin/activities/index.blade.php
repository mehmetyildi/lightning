		...
		<div class="x_panel">
			<div class="x_title">
				...
				<ul class="nav navbar-right panel_toolbox">
					...
					<a type="button" class="btn btn-primary" href="/activity/create">Yeni Ekle</a>
				</ul>
				<div class="clearfix"></div>
			</div>
			<div class="x_content">
				<p class="text-muted font-13 m-b-30">
					Tüm blog etkinlikler bu listede en yeniden en eskiye doğru sıralı olarak gösterilmiştir. Etkinlikler arasında arama yapabilir, değişiklik ya da silme işlemi uygulayabilirsiniz.
				</p>
				<table id="datatable" class="table table-striped table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{$i}}</td>
							<td><img style="max-height: 100px; max-width: 150px" src="{{$activity->image_url}}"></td>

							<td>{{$activity->title}}</td>
							<td>{{$activity->user->name}}</td>

							<td>@if($activity->publish==1)
							...
							<td>
								<a href="/activity/edit/{{$activity->id}}"><button class="btn btn-xs btn-info"><i class="glyphicon glyphicon-book"></i>Düzenle</button></a>
								<form method="POST" action="/activity/delete/{{$activity->id}}" accept-charset="UTF-8" style="display:inline">
									{{csrf_field()}}
									<button class="btn btn-xs btn-danger" type="button" data-toggle="modal" data-target="#confirmDelete" data-title="Makaleyi Sil" data-message="{{$activity->title}} adlı etkinliği kalıcı olarak silmek istediğinize emin misiniz?">
