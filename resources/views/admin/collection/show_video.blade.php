...
<!-- 	<div class="post-share">
	...
	<div class="postdtl-info">
		<p class="post-category"><a href="/videos/categories/{{$video->category->id}}">{{$video->category->title}}</a></p>
		<h3 class="post-title"><a href="#">{{$video->title}}</a></h3>
		<div class="post-meta">
			<span><a href="/videos/user/{{$video->user->id}}"><i class="fa fa-user"></i> {{$video->user->name}}</a></span>
...
<!-- 	<div class="comment-box">
	...
	<form method="POST" action="/video/confirm/{{$video->id}}" accept-charset="UTF-8" style="display:inline">
		{{csrf_field()}}
		<button class="btn btn-primary" type="button" data-toggle="modal" data-target="#confirm_post" data-title="Makaleyi Yayımla" data-message="{{$video->title}} adlı makaleyi yayımlamak istediğinize emin misiniz?">
			Onayla&nbsp<i class="fa fa-thumbs-up"></i>
		</button>
	</form>

	<form method="POST" action="/video/revise/{{$video->id}}" accept-charset="UTF-8" style="display:inline">
		{{csrf_field()}}
		<div hidden="true">
	...
	<form method="POST" action="/video/destroy/{{$video->id}}" accept-charset="UTF-8" style="display:inline">
		{{csrf_field()}}
		<button class="btn btn-primary" type="button" data-toggle="modal" data-target="#confirmDelete" data-title="Makaleyi Sil" data-message="{{$video->title}} adlı makaleyi kalıcı olarak silmek istediğinize emin misiniz?">
