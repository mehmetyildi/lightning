...
<!-- 	<div class="post-share">
	...
	<div class="postdtl-info">
		<p class="post-category"><a href="/book/categories/{{$book->category->id}}">{{$book->category->title}}</a></p>
		<h3 class="post-title"><a href="#">{{$book->title}}</a></h3>
		<div class="post-meta">
			<span><a href="/posts/user/{{$book->user->id}}"><i class="fa fa-user"></i> {{$book->user->name}}</a></span>
			<span><a href="#"><i class="fa fa-clock-o"></i> {{$book->created_at->diffForHumans()}}</a></span>
		</div>
		@if($book->age_begin)
...
<!-- 	<div class="comment-box">
	...
	<form method="POST" action="/book/confirm/{{$book->id}}" accept-charset="UTF-8" style="display:inline">
		{{csrf_field()}}
		<button class="btn btn-primary" type="button" data-toggle="modal" data-target="#confirm_post" data-title="Makaleyi Yayımla" data-message="{{$book->title}} adlı makaleyi yayımlamak istediğinize emin misiniz?">
			Onayla&nbsp<i class="fa fa-thumbs-up"></i>
		</button>
	</form>

	<form method="POST" action="/book/revise/{{$book->id}}" accept-charset="UTF-8" style="display:inline">
		{{csrf_field()}}
		<div hidden="true">
	...
	<form method="POST" action="/book/destroy/{{$book->id}}" accept-charset="UTF-8" style="display:inline">
		{{csrf_field()}}
		<button class="btn btn-primary" type="button" data-toggle="modal" data-target="#confirmDelete" data-title="Makaleyi Sil" data-message="{{$book->title}} adlı makaleyi kalıcı olarak silmek istediğinize emin misiniz?">
