		...
		<div class="x_panel">
			<div class="x_title">
				...
				<ul class="nav navbar-right panel_toolbox">
					<a type="button" class="btn btn-primary" href="/book/create">Yeni Ekle</a>
			...
			<div class="x_content">
				<p class="text-muted font-13 m-b-30">
					Tüm blog kitap tanıtımları bu listede en yeniden en eskiye doğru sıralı olarak gösterilmiştir. Bunlar arasında arama yapabilir, değişiklik ya da silme işlemi uygulayabilirsiniz.
				</p>
				<table id="datatable" class="table table-striped table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{$i}}</td>
							<td><img style="max-height: 100px; max-width: 150px" src="{{$book->image_url}}"></td>

							<td>{{$book->title}}</td>
							<td>{{$book->user->name}}</td>

							<td>@if($book->publish==1)
							...
							<td>
								<a href="/book/edit/{{$book->id}}"><button class="btn btn-info btn-xs"><i class="glyphicon glyphicon-book"></i>Düzenle </button></a>
								<form method="POST" action="/book/delete/{{$book->id}}" accept-charset="UTF-8" style="display:inline">
									{{csrf_field()}}
									<button class="btn btn-xs btn-danger" type="button" data-toggle="modal" data-target="#confirmDelete" data-title="Makaleyi Sil" data-message="{{$book->title}} adlı kitap tanıtımını kalıcı olarak silmek istediğinize emin misiniz?">
