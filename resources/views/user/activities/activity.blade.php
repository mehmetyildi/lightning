...
<meta property="og:url" content="http://localhost:8000/{{$activity->type()}}/show/{{$activity->url}}" />
	...
	<div class="post-share">
		<div class="social-share col-md-12">
			<div class="col-md-2 col-xs-4 ">
				<a class="activity-like like"><i class="fa fa-2x fa-thumbs-up "></i></a><p>{{$activity->like}}</p>
			</div>
			<div class="col-md-2 col-xs-4">
				<a class="activity-dislike dislike" ><i class="fa fa-2x fa-thumbs-down "></i></a><p>{{$activity->dislike}}</p>
			</div>
			<div class="col-md-2 col-xs-4">
				<i class="fa fa-2x fa-desktop "></i><p>{{$activity->hit}}</p>
			</div>
		</div>
		
	</div>
	<div class="postdtl-info">
		<p class="post-category"><a href="/collection/categories/{{$activity->category->url}}">{{$activity->category->title}}</a></p>
		<h1 class="post-title"><a href="#">{{$activity->title}}</a></h1>
		<div class="post-meta">
			<span><a href="/collection/user/{{$activity->user->url}}"><i class="fa fa-user"></i> {{$activity->user->name}}</a></span>
			<span><a href="#"><i class="fa fa-clock-o"></i> {{$activity->approved_at->diffForHumans()}}</a></span>
		</div>
...
<!-- 	<div class="comment-box">
	...
	<div class="comment-box">
		<div class="title">
			@if(count($activity->comments))
			<h2>Bu makale hakkında {{count($activity->comments) }} yorum yapılmış.</h2>
			@else
			<h2>Bu makale hakkında henüz yorum yapılmamış. İlk yorumu yapan siz olun</h2>
			@endif
		</div>

		@if(is_array($activity->comments)||is_object($activity->comments))
		...
		<div class="single-comment-box">
			...
			<form hidden="true" action="/comment/{{$comment->id}}/related_comments" method="POST" class=" contact-form row related_comment" id="contact-page-contact-form" novalidate="novalidate">
				...
				<input hidden="true" type="text" name="activity_id" value="{{$activity->id}}">
				<div class="col-md-12">
	...
	<div class="comment-form contact-content">
		...
		<form action="/activity/{{$activity->id}}/comments" method="POST" class="contact-form row" id="contact-page-contact-form" novalidate="novalidate">
			{{csrf_field()}}
			<input hidden="true" type="text" name="user_id" value="{{auth()->user()->id}}">
			<div class="col-md-12">
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.activity-like').one('click',function(){

			$.getJSON("/get/activity/like/{{$activity->id}}",function(data){
		...
		$('.activity-dislike').one('click',function(){

			$.getJSON("/get/activity/dislike/{{$activity->id}}",function(data){
